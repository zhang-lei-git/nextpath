...
MVP 是模块化单体：一个可独立部署的服务，模块边界已经明确。成绩确认先以幂等方式保存并创建分析任务，家长端可看到处理中状态；分析与报告完成后原子发布，避免出现“成绩已保存但报告永久缺失”的断链。OCR、批量文件解析、网页采集、分析报告和大规模重算通过独立 worker / 队列消费者运行。生产使用 PostgreSQL、Redis 和对象存储；SQLite 只用于内部演示。
