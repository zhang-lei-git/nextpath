const state = { sources: [], evidence: [], facts: [], releases: [], publishedFacts: [], users: [], models: [], validations: [], calibrationSamples: [], ingestions: [], collectionJobs: [], collectionRuns: [], alerts: [], governanceRules: [], selectedCollectionRun: null, activeStatus: "pending_review", activeAlertStatus: "open", selectedReleaseId: "", selectedModelId: "" };
const titles = { overview: "数据总览", data: "基础数据", sources: "数据来源", capture: "采集录入", review: "审核发布", quality: "质量治理", models: "分析模型", users: "用户管理" };
...
function badge(status) { return `<span class="badge ${status}">${({pending_review:"待审核",approved:"已通过",rejected:"已拒绝",running:"运行中",normalized:"已治理",unchanged:"无变化",failed:"失败",active:"已启用",inactive:"未启用",paused:"已暂停",open:"待处理",resolved:"已处理",high:"高",medium:"中",low:"低"})[status] || status}</span>`; }
...
async function load() {
  const [sources, evidence, facts, releases, me, users, models, calibrationSamples, ingestions, collectionJobs, collectionRuns, alerts, governanceRules] = await Promise.all([dataApi("sources"), dataApi("evidence"), dataApi("facts"), dataApi("releases"), api("auth/me"), api("users"), analysisApi("models"), analysisApi("calibration-samples"), dataApi("ingestions"), dataApi("collection-jobs"), dataApi("collection-runs"), dataApi("alerts"), dataApi("governance-rules")]);
  Object.assign(state, { sources, evidence, facts, releases, users, models, calibrationSamples, ingestions, collectionJobs, collectionRuns, alerts, governanceRules, selectedReleaseId: state.selectedReleaseId || releases[0]?.id || "", selectedModelId: state.selectedModelId || models[0]?.id || "" });
...
function render() { renderOverview(); renderSources(); renderSelectors(); renderFacts(); renderPublishedFacts(); renderIngestions(); renderCollectionJobs(); renderCollectionRuns(); renderQuality(); renderModels(); renderUsers(); }
function renderOverview() {
  ...
  $("#metrics").innerHTML = [["数据来源",state.sources.length,"已登记"],["待审核",pending,"暂未进入家长端"],["运营待办",openAlerts,"需要处理"],["已发布版本",state.releases.length,"家长端可读取"]].map(([label,value,note]) => `<div class="metric"><span>${label}</span><strong>${value}</strong><small>${note}</small></div>`).join("");
...
function renderCollectionRunDetail() {
  ...
  const snapshots = detail.snapshots.map((snapshot) => {
    const summary = snapshot.diff_summary || {};
    const attachments = summary.attachments || [];
    const diff = summary.text_diff || {};
    const attachmentList = attachments.length ? `<div class="attachment-list">${attachments.map((item)=>`<p>${item.error ? "未保存" : "已保存"} · ${text(item.url)}${item.size ? ` · ${Math.ceil(item.size/1024)}KB` : ""}${item.error ? ` · ${text(item.error)}` : ""}</p>`).join("")}</div>` : "";
    const textChanges = diff.similarity === null || diff.similarity === undefined ? "" : `<div class="change-summary"><strong>正文变化 · 相似度 ${Math.round(diff.similarity * 100)}%</strong>${diff.added?.length ? `<div class="change-group added"><span>新增 ${diff.added_count} 处</span>${diff.added.map((item) => `<p>${text(item)}</p>`).join("")}</div>` : ""}${diff.removed?.length ? `<div class="change-group removed"><span>删除 ${diff.removed_count} 处</span>${diff.removed.map((item) => `<p>${text(item)}</p>`).join("")}</div>` : ""}</div>`;
    return `<div class="detail-block"><strong>原始快照 · ${text(changeType(snapshot.change_type))}</strong><p>${text(snapshot.final_url || snapshot.source_url)}</p><dl><div><dt>响应状态</dt><dd>${snapshot.response_status ?? "-"}</dd></div><div><dt>内容哈希</dt><dd class="mono">${text(snapshot.content_hash || "-")}</dd></div><div><dt>结构变化</dt><dd>${summary.structure_changed ? "有" : "无"}</dd></div><div><dt>附件</dt><dd>${attachments.length} 个</dd></div></dl>${textChanges}${attachmentList}</div>`;
  }).join("");
...
function renderQuality() {
  const visibleAlerts = state.alerts.filter((alert) => alert.status === state.activeAlertStatus);
  $("#alertList").innerHTML = visibleAlerts.length ? visibleAlerts.map((alert) => `<div class="list-item alert-item ${text(alert.severity)}"><div class="item-title-row"><strong>${text(alert.title)}</strong><div>${badge(alert.severity)} ${badge(alert.status)}</div></div><p>${formatDate(alert.created_at)}</p><p>${text(alert.details.region || "")} ${alert.details.reference_year || ""}</p><div class="actions"><button class="button small secondary" data-alert-id="${alert.id}" data-alert-next="${alert.status === "open" ? "resolved" : "open"}">${alert.status === "open" ? "标记已处理" : "重新打开"}</button>${alert.run_id ? `<button class="button small secondary" data-alert-run="${alert.run_id}">查看采集记录</button>` : ""}</div></div>`).join("") : `<div class="empty">${state.activeAlertStatus === "open" ? "当前没有待处理事项" : "还没有已处理事项"}</div>`;
  $("#governanceRuleCount").textContent = `${state.governanceRules.length} 个版本`;
  $("#governanceRuleList").innerHTML = state.governanceRules.length ? state.governanceRules.map((rule) => `<article class="rule-card"><div class="item-title-row"><strong>${text(rule.name)}</strong>${badge(rule.status)}</div><p class="mono">${text(rule.version)}</p><dl><div><dt>字段别名</dt><dd>${Object.keys(rule.rules.field_aliases || {}).length}</dd></div><div><dt>实体别名</dt><dd>${Object.keys(rule.rules.entity_aliases || {}).length}</dd></div><div><dt>允许类别</dt><dd>${(rule.rules.allowed_fact_types || []).length || "全部"}</dd></div></dl><p>${formatDate(rule.created_at)} · ${text(rule.created_by)}</p></article>`).join("") : '<div class="empty">当前使用系统基础规则，可按地区和数据类型建立独立版本。</div>';
  $$('[data-alert-id]').forEach((button) => button.addEventListener("click", () => updateAlert(button.dataset.alertId, button.dataset.alertNext)));
  $$('[data-alert-run]').forEach((button) => button.addEventListener("click", async () => { await showCollectionRun(button.dataset.alertRun); document.querySelector('[data-view="capture"]').click(); }));
}
...
async function updateAlert(id, status) { try { await dataApi(`alerts/${id}`, {method:"PATCH",body:JSON.stringify({status})}); notice(status === "resolved" ? "待办已处理" : "待办已重新打开"); await load(); } catch(error) { notice(error.message,true); } }
...
$("#sourceForm").addEventListener("submit", async (event) => { event.preventDefault(); const form=event.currentTarget; const body=formData(form); if (!body.homepage_url) body.homepage_url=null; try { await dataApi("sources", {method:"POST",body:JSON.stringify(body)}); form.reset(); notice("数据来源已登记"); await load(); } catch(error) { notice(error.message,true); } });
$("#evidenceForm").addEventListener("submit", async (event) => { event.preventDefault(); const form=event.currentTarget; const body=formData(form); ["url","excerpt"].forEach((key) => { if (!body[key]) body[key]=null; }); try { await dataApi("evidence", {method:"POST",body:JSON.stringify(body)}); form.reset(); notice("证据已保存，可继续录入候选数据"); await load(); } catch(error) { notice(error.message,true); } });
$("#documentForm").addEventListener("submit", async (event) => { event.preventDefault(); const form=event.currentTarget; const payload=new FormData(form); if (!payload.get("source_id")) payload.delete("source_id"); try { const response = await fetch("/api/data/ingestions/documents", { method:"POST", body:payload }); const result = await response.json(); if (!response.ok) throw new Error(result.detail || "上传失败"); form.reset(); notice("资料已保存，等待治理"); await load(); } catch(error) { notice(error.message,true); } });
$("#collectionForm").addEventListener("submit", async (event) => { event.preventDefault(); const body=formData(event.currentTarget); const jobId=body.job_id; delete body.job_id; body.interval_minutes=Number(body.interval_minutes); if (!body.source_id) body.source_id=null; if (!body.region) body.region=null; if (!body.data_type) body.data_type=null; if (!body.governance_rule_version) body.governance_rule_version=null; if (!body.extraction_hint) body.extraction_hint=null; try { await dataApi(jobId ? `collection-jobs/${jobId}` : "collection-jobs", {method:jobId ? "PATCH" : "POST",body:JSON.stringify(body)}); resetCollectionForm(); notice(jobId ? "采集任务已更新" : "采集任务已保存"); await load(); } catch(error) { notice(error.message,true); } });
$("#cancelCollectionEdit").addEventListener("click", resetCollectionForm);
$("#factForm").addEventListener("submit", async (event) => { event.preventDefault(); const form=event.currentTarget; const body=formData(form); try { body.scope = body.scope ? JSON.parse(body.scope) : {}; body.value = JSON.parse(body.value); body.reference_year = Number(body.reference_year); body.evidence_ids = [body.evidence_id]; delete body.evidence_id; await dataApi("facts", {method:"POST",body:JSON.stringify(body)}); form.reset(); notice("候选数据已提交，等待审核"); await load(); } catch(error) { notice(error.message.includes("JSON") ? "适用范围或数据内容不是有效 JSON" : error.message,true); } });
...
$("#userForm").addEventListener("submit", async (event) => { event.preventDefault(); const form=event.currentTarget; try { await api("users", { method:"POST", body:JSON.stringify(formData(form)) }); form.reset(); notice("用户已添加"); await load(); } catch(error) { notice(error.message,true); } });
$("#governanceRuleForm").addEventListener("submit", async (event) => { event.preventDefault(); const form=event.currentTarget; const body=formData(form); try { body.rules=JSON.parse(body.rules); body.status="active"; await dataApi("governance-rules", {method:"POST",body:JSON.stringify(body)}); form.reset(); notice("治理规则版本已保存"); await load(); } catch(error) { notice(error instanceof SyntaxError ? "规则配置不是有效 JSON" : error.message,true); } });
...
$("#calibrationForm").addEventListener("submit", async (event) => { event.preventDefault(); const form=event.currentTarget; const body=formData(form); ["cohort_year", "grade_rank", "grade_size", "final_city_rank"].forEach((key) => body[key]=Number(body[key])); body.final_candidate_count = body.final_candidate_count ? Number(body.final_candidate_count) : null; if (!body.source_note) body.source_note=null; body.evidence_ids=[]; try { await analysisApi("calibration-samples", { method:"POST", body:JSON.stringify(body) }); form.reset(); notice("校准样本已提交，审核通过后才会参与分析"); await load(); } catch(error) { notice(error.message,true); } });
$("#validationForm").addEventListener("submit", async (event) => { event.preventDefault(); const form=event.currentTarget; const body=formData(form); ["validation_year", "sample_size"].forEach((key) => body[key]=Number(body[key])); ["median_absolute_rank_error", "interval_coverage"].forEach((key) => body[key]=body[key] === "" ? null : Number(body[key])); if (!body.notes) body.notes=null; try { await analysisApi(`models/${state.selectedModelId}/validations`, { method:"POST", body:JSON.stringify(body) }); form.reset(); notice("验证记录已保存"); await load(); } catch(error) { notice(error.message,true); } });
...
$$("[data-alert-status]").forEach((button) => button.addEventListener("click", () => { state.activeAlertStatus=button.dataset.alertStatus; $$("[data-alert-status]").forEach((item)=>item.classList.toggle("selected",item===button)); renderQuality(); }));
