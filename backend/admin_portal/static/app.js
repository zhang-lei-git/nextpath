const state = { sources: [], evidence: [], facts: [], releases: [], publishedFacts: [], users: [], activeStatus: "pending_review", selectedReleaseId: "" };
const titles = { overview: "数据总览", data: "基础数据", sources: "数据来源", capture: "采集录入", review: "审核发布", users: "用户管理" };
...
async function load() {
  const [sources, evidence, facts, releases, me, users] = await Promise.all([dataApi("sources"), dataApi("evidence"), dataApi("facts"), dataApi("releases"), api("auth/me"), api("users")]);
  Object.assign(state, { sources, evidence, facts, releases, users, selectedReleaseId: state.selectedReleaseId || releases[0]?.id || "" });
...
function render() { renderOverview(); renderSources(); renderSelectors(); renderFacts(); renderPublishedFacts(); renderUsers(); }
